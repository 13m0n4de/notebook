...
物理内存也由一组固定大小的槽块组成，下面的例子中有 8 个 **页帧**，虚拟地址空间的页放在物理内存的不同位置。
...
分页有许多优点，最大的改进就是 **灵活性**：通过完善的分页方法，操作系统能够高效地提供地址空间的抽象，不管进程如何使用地址空间。例如，我们不会假定堆和栈的增长方向，以及它们如何使用。
...
虚拟地址 `21` 在虚拟页 `01` 的偏移量 `0101`（十进制 `5`）处。
