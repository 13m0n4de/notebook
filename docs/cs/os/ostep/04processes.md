...
!!! note "Answer"
    ...
    `5` 是指令数量，`100` 是 **不切换** I/O 的机率，所以这是两个五条指令的纯 CPU 程序，它们应该会一直使用 CPU，利用率为 100%。
...
!!! note "Answer"
    ...
    I/O 操作时长默认值是 `5`，所以一共需要 `4 + 7 = 11` 个时间单位，因为 `io` 和 `io_done` 操作也占用时间，所以是 `7` 不是 `5`。
...
!!! note "Answer"
    ...
    如果先 `RUN:io` 的话，就会节省很多时间，因为之前程序 0 结束了才开始 I/O，浪费了一些。
...
!!! note "Answer"
    ...
    如果在 I/O 操作的程序不让出 CPU 的话，其他程序只能一直等待了。
...
!!! note "Answer"
    ...
    浪费了一些系统资源，程序 2 和 3 一直占用着 CPU，导致程序 1 没法进行下一个 I/O 操作，如果 I/O 操作 与 CPU 操作一起执行就会更节省。
